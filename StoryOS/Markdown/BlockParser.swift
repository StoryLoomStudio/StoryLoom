//
//  Vendored from SwiftMarkdownEngine (Apache 2.0) — see Markdown/NOTICE.md.
//  Modified for StoryLoom: `nonisolated` on the parser types.
//
//  BlockParser.swift
//  MarkdownEngine
//
...
nonisolated enum MarkdownBlockKind: Equatable {
...
nonisolated struct MarkdownBlock: Equatable {
...
nonisolated enum BlockParser {

    private static let cacheLock = NSLock()
    nonisolated(unsafe) private static var cachedChars: [unichar]?     // UTF-16 buffer of the last parse
    nonisolated(unsafe) private static var cachedBlocks: [MarkdownBlock]?
    /// Registry fingerprint the memo was computed under — extension fences
    /// change the block structure of identical text.
    nonisolated(unsafe) private static var cachedFingerprint: String = ""
    ...
    static func parse(_ text: String, utf16Chars: [unichar]? = nil, registry: ExtensionRegistry = .empty) -> [MarkdownBlock] {
    ...
    static func seedCache(chars: [unichar], blocks: [MarkdownBlock], fingerprint: String = "") {
    ...
    static func incrementalParse(oldChars o: [unichar], oldBlocks: [MarkdownBlock], newChars n: [unichar], newNS: NSString, diff: BufferDiff, registry: ExtensionRegistry = .empty) -> (blocks: [MarkdownBlock], window: Int)? {
    ...
    static func computeBlocks(_ text: String, registry: ExtensionRegistry = .empty) -> [MarkdownBlock] {
        ...
        while i < lines.count {
            ...
            if isBlank(line) {
                ...
                blocks.append(MarkdownBlock(kind: .blank, range: union(lines[i...end])))
                i = end + 1

            } else if isFence(line), let end = fenceCloseIndex(from: i) {
                blocks.append(MarkdownBlock(kind: .fencedCode, range: union(lines[i...end])))
                i = end + 1

            } else if isThematicBreak(line) {
                blocks.append(MarkdownBlock(kind: .thematicBreak, range: lines[i]))
                i += 1

            } else if isHeading(line) {
                blocks.append(MarkdownBlock(kind: .heading, range: lines[i]))
                i += 1

            } else if isBlockquote(line) {
                var end = i
                while end + 1 < lines.count, isBlockquote(lineText(end + 1)) { end += 1 }
                blocks.append(MarkdownBlock(kind: .blockquote, range: union(lines[i...end])))
                i = end + 1
                ...
                blocks.append(MarkdownBlock(kind: .list, range: union(lines[i...end])))
                i = end + 1
                ...
                blocks.append(MarkdownBlock(kind: .table, range: union(lines[i...end])))
                i = end + 1

            } else if isBlockLatexOpen(line), let end = blockLatexCloseIndex(from: i) {
                // MarkdownBlock LaTeX `$$…$$` — a single line or a `$$`-delimited run.
                blocks.append(MarkdownBlock(kind: .blockLatex, range: union(lines[i...end])))
                i = end + 1
                ...
                blocks.append(MarkdownBlock(kind: .ext(entry.id), range: union(lines[i...end])))
                i = end + 1
                ...
                blocks.append(MarkdownBlock(kind: .paragraph, range: union(lines[i...end])))
                i = end + 1
            }
        }
        return blocks
...
nonisolated private extension MarkdownBlock {
    /// A copy with the range moved by `d` UTF-16 units.
    func shifted(by d: Int) -> MarkdownBlock {
        MarkdownBlock(kind: kind, range: NSRange(location: range.location + d, length: range.length))
    }
}
